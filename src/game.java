...
//        设置棋局的相关选项
        ...
        group.add(perButton);group.add(comButton);
        j2.add(perButton);j2.add(comButton);
...
//        退出
    ...
    public void play(MouseEvent e){
        if(type!=-1){
            ...
            if(x2>=0&&y2>=0&&x2<=cBoard.getcBoardSize()&&y2<=cBoard.getcBoardSize()&&gameArray[y2][x2]==0){
...
//                    有一方赢了
                    if (person==1)  JOptionPane.showMessageDialog(null,"黑子赢了","tip",JOptionPane.PLAIN_MESSAGE);
                    else JOptionPane.showMessageDialog(null,"白子赢了", "tip",JOptionPane.PLAIN_MESSAGE);
                    end();
                }
            }
        }
    ...
    public boolean ifWin(int x,int y,int type){
        ...
        if(a1.indexOf(com)!=-1) return true;
        String a2 = getString2(y);
        if(a2.indexOf(com)!=-1) return true;
        String a3 = getString3(x,y);
        if(a3.indexOf(com)!=-1) return true;
        String a4 = getString4(x,y);
        if(a4.indexOf(com)!=-1) return true;
        return false;
...
//        清空落子集合
    ...
    public String getString2(int y){
        String s="";
        for(int i=0;i<gameArray.length;i++) s+=gameArray[i][y];
        return  s;
    }
    /*将撇方向转字符串*/
    public String getString3(int x,int y){
        ...
        if(x<=y){
            int gap = y-x;
            for(int i=0;i+gap<=18;i++) s+=gameArray[i][i+gap];
        }else{
            int gap = x-y;
            for(int i=0;i+gap<=18;i++) s+=gameArray[gap+i][i];
        }
    ...
    public String getString4(int x,int y){
        ...
        if(x+y<=18){
            int gap = y+x;
            for(int i=gap;i>=0;i--) s+=gameArray[i][gap-i];
        }else{
            int gap = x+y;
            for(int i=18;gap-i<=18;i--) s+=gameArray[i][gap-i];
        }
                ...
                if(gameArray[i][j]!=0){
                    ...
                    for(int a=0;a<xx.length;a++){
                        for(int b=0;b<xx.length;b++){
                            if(ifBorder(i+xx[a],j+xx[b])) continue;
...
//        捺
        ...
        int cout[] =new int[11];
        for(int i=0;i<11;i++) cout[i] = 0;
        cout[a1]++;cout[a2]++;cout[a3]++;cout[a4]++;

        if(a1==10&a2==10&&a3==10&&a4==10) return 0;
        if(cout[0]>=1) {
...
//            System.out.println(Arrays.toString(cout));
        ...
        if(cout[4]>=1) return 50;
        if(cout[5]>=1&&cout[6]>=1) return 10;
        if(cout[5]>=1) return 5;
        if(cout[6]>=1) return 3;
        if(cout[7]>=1) return -5;
        if(cout[8]>=1) return -5;
        if(cout[9]>=1) return -5;
        return 0;
    }
